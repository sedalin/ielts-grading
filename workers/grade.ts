...
async function callMiniMax(messages: any[], env: Env): Promise<string> {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!topic || !content) {
      return new Response(JSON.stringify({ error: 'Missing topic or content' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
...
请根据以上标准进行严格批改。`;
    ...
    try {
      const result = await callMiniMax(messages, env);
